...
CONFIG_PATH = 'configs/baseline_3.yml'
CHECKPOINT_PATH = ...
RESUME_TRAIN_PATH = ...
...
def fit(config_path, best_model_path=None, resume_train_path=None):
    ...
    if resume_train_path:
        ...
        model, optimizer, loaded_config, exp_dir, start_epoch, best_val_acc = load_checkpoint(resume_train_path, model, optimizer, device)
        config = loaded_config 
        logger = setup_logging(exp_dir)
        logger.info(f"Resumed training from checkpoint: {resume_train_path}")
